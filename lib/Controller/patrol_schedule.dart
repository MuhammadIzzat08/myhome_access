...
class PatrolScheduleController {
  ...
  Stream<List<PatrolScheduleModel>> getPatrolSchedule() {
    String currentDate = _getCurrentDate(); // Get today's date
    return _firestore
        .collection('patrolSchedule')
        .where('day', isGreaterThanOrEqualTo: currentDate) // Filter by date >= today's date
        .orderBy('day', descending: false) // Order by date in ascending order
        .snapshots()
        .map((snapshot) {
      return snapshot.docs.map((doc) => PatrolScheduleModel.fromMap(doc.id, doc.data())).toList();
    });
  }
